...
public abstract class DoubleEndedSinglyLinkedStructure<T> extends AbstractLinkedStructure<T> {
    protected Node<T> head = null; // First element, null if empty
    private Node tail = null; // Last element, null if empty

    protected DoubleEndedSinglyLinkedStructure(String type) {
        super(type);
    }
    ...
    protected Node<T> removeHead() {
        ...
        if (tail == head) tail = null; // Clear tail, if our head and tail are the same
    ...
    protected Node<T> removeAtIndex(int index) {
        ...
        if (oldNode.next == null)  tail = previous; // Update tail
     ...
     * This is O(n), but would be O(1) in a doubly linked list.
     *
    ...
    protected Node<T> getNodeBeforeTail() {
        ...
        endsub("Found the tail of the structure, #"+(i+1)+": '"+tail+"'.");
        return current;
    }
    ...
    public void printAll() {
        ...
        while(node != null) {
            ...
            else if (node == tail) System.out.print(" (Tail)");
            System.out.println();
